...
resource "formal_sidecar" "main" {
  name       = var.name
  technology = "bigquery"
  hostname   = var.bigquery_sidecar_hostname
}

resource "formal_resource" "main" {
  technology = "bigquery"
  name       = var.name
  hostname   = var.bigquery_hostname
  port       = var.main_port
}
...
# Native Role
resource "formal_native_user" "main_bigquery" {
  resource_id        = formal_resource.main.id
  native_user_id     = var.bigquery_username
  native_user_secret = var.bigquery_password
